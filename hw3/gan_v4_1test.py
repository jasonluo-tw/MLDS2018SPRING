...
class image_gan():
    ...
    def build_model(self):
        ...
        ones_ = tf.random_uniform(tf.shape(self.Dx), minval=0.7, maxval=1.2)
        zeros_ = tf.random_uniform(tf.shape(self.Dg), minval=0.0, maxval=0.3)
        ones_1 = tf.random_uniform(tf.shape(self.Dg), minval=0.7, maxval=1.2)
        ...
        self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = self.Dx, labels = ones_))
        self.d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = self.Dg, labels = zeros_))
        self.d_total_loss = self.d_loss_real + self.d_loss_fake
        # Loss function for generator
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits = self.Dg, labels = ones_1))
        ...
        self.d_trainer = tf.train.AdamOptimizer(0.0002, beta1=0.5).minimize(self.d_total_loss, var_list=d_vars)

        # Train approch for the generator
        self.g_trainer = tf.train.AdamOptimizer(0.0002, beta1=0.5).minimize(self.g_loss, var_list=g_vars)
