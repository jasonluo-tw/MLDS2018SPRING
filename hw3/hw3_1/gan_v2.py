...
class image_gan():
    ...
    def build_model(self):
        ...
        self.d_trainer = tf.train.AdamOptimizer(0.0003).minimize(self.d_total_loss, var_list=d_vars)

        # Train approch for the generator
        self.g_trainer = tf.train.AdamOptimizer(0.0001).minimize(self.g_loss, var_list=g_vars)
    ...
    def discriminator(self, images, reuse_variables=None):
        ...
        with tf.variable_scope(tf.get_variable_scope(), reuse=reuse_variables) as scope:
            ...
            d_w1, d_b1 = self.weights('d_w1', [5, 5, 3,  32])
    ...
    def generator(self, z):
        ...
        # Dimension of g4: batch_size x 64 x 64 x 3 
        return g5
...
if __name__ == '__main__':
    ...
    batch_size = 100
    z_dimension = 100
    gangan = image_gan(batch_size)
    gangan.build_model()

    z_batch0 = np.random.normal(0, 1, [64, z_dimension]) # for test images
    ...
    with tf.Session() as sess:
        ...
        for i in range(300):
            ...
            loss_real, loss_fake = gangan.train_discriminator(real_image_batch, z_batch)
            
            if(i % 100 == 0):
                print("dLossReal:", loss_real, "dLossFake:", loss_fake)
        
        # Train generator and discriminator together
        for itera in tqdm(range(20000)):
            real_image_batch = mnist.train.next_batch(batch_size)[0].reshape([batch_size, 28, 28, 1])
            z_batch = np.random.normal(0, 1, size=[batch_size, z_dimension])
            ...
            # Train generator
            z_batch = np.random.normal(0, 1, size=[batch_size, z_dimension])
            gg_loss = gangan.train_generator(z_batch)

            if itera % 1000 == 0:
                print("dLossReal:", loss_real, "dLossFake:", loss_fake)

                # for generate images
                testImage = gangan.generate_image(z_batch0)
                testImage = testImage.reshape([64, 28, 28])
                for jj, im in enumerate(testImage):
                    plt.subplot(8, 8, jj+1)
                    plt.axis('off')
                    plt.imshow(im, cmap='Greys')
                #plt.show()
                plt.savefig('img_v2/test_img'+str(itera)+'.png')
